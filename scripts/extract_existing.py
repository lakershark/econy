"""
Re-run extraction (TOC + briefing + summaries) against an EXISTING NotebookLM
notebook, without creating a new notebook or re-uploading the PDF.

Use when a notebook + PDF source already exist (status ready) but the issue
JSON has an empty/missing toc — i.e. the pipeline's create/upload steps
succeeded but extraction never wrote back.

Usage: python3 scripts/extract_existing.py <notebook_id> <magazine> <date>
...
# Reuse the helpers already defined in pipeline.py
from pipeline import (
    notebooklm, run, extract_json_block, normalize, get_answer, ROOT,
)


def extract(nb_id, magazine, date):
    ...
    print(f'\n{"="*60}')
    print(f'Re-extracting (existing notebook): {mag_label} {date}')
    ...
    if not toc_data:
        print('   ERROR: Could not parse TOC. Aborting (not overwriting JSON).')
        return None
    ...
    if total_articles == 0:
        print('   ERROR: TOC empty. Aborting (not overwriting JSON).')
        return None
    ...
    def build_summary_prompt(articles_slice, mag):
        ...
        lines = [f'[{section}] {art["title"]}' for section, art in articles_slice]
        return header + '\n'.join(lines)
...
if __name__ == '__main__':
    nb_id, magazine, date = sys.argv[1], sys.argv[2], sys.argv[3]
    extract(nb_id, magazine, date)
